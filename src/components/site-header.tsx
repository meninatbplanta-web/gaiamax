    ...
    <header className="relative border-b border-slate-200 bg-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        ...
        <nav className="hidden items-center gap-5 text-sm font-medium text-slate-600 md:flex">
          ...
          <Link href="/forum" className="hover:text-brand">Fórum</Link>

          {user ? (
            <>
              <Link href="/meus-cursos" className="hover:text-brand">Meus cursos</Link>
              <Link href="/suporte" className="hover:text-brand">Suporte</Link>
              {isAdmin ? <Link href="/admin" className="hover:text-brand">Admin</Link> : null}
              <Link href="/conta" className="hover:text-brand">{accountLabel}</Link>
              <form action={signOut}>
        ...
        <MobileNav loggedIn={!!user} isAdmin={isAdmin} accountLabel={accountLabel} />
      </div>
    </header>
  );
}
